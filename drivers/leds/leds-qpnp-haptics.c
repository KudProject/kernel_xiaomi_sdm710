...
static int qpnp_haptics_auto_mode_config(struct hap_chip *chip, int time_ms)
{
	...
	if (time_ms <= 20) {
		...
		if (chip->effect_max) {
			...
			if (chip->effect_index != index) {
				if (chip->effect_max >= index)
					index = chip->effect_max - 1;
				chip->effect_index = index;
				for (i = 0; i < HAP_WAVE_SAMP_LEN; i++)
					wave_samp[i] = (u32)(
						chip->effect_array[index][i]);
...
static int qpnp_haptics_parse_buffer_dt(struct hap_chip *chip)
{
	...
	if (!rc) {
		...
		if (!prop)
			dev_err(&chip->pdev->dev, "effect array not found\n");
		else if (HAP_WAVE_SAMP_LEN * chip->effect_max != temp) {
			dev_err(&chip->pdev->dev,
				"Invalid len of effect arry\n");
			chip->effect_max = 0;
			return -EINVAL;
		} else {
