			...
			uint32_t param_id, char *params, uint32_t params_length)
{
	...
	*update_param_data++ = params_length;
	...
	memcpy((char *)param_data + sizeof(struct adm_param_data_v5), params,
			params_length);

	ret = adm_send_params_v5(port_id, copp_idx, (char *)param_data,
				param_size);
	if (ret) {
			...
			uint32_t client_id)
{
	...
	ret = adm_get_params(port_id, copp_idx, module_id, param_id,
