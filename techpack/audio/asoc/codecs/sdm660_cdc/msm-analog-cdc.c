						...
						bool enable)
{
	...
	if (atomic_inc_return(&supply->ref) == 1) {
		ret = regulator_set_voltage(supply->supply, supply->min_uv,
						supply->max_uv);
		ret += regulator_set_load(supply->supply, supply->optimum_ua);
					     ...
					     int event)
{
	...
	case SND_SOC_DAPM_POST_PMD:
		if (!sdm660_cdc->micb1_always_on)
			snd_soc_update_bits(codec,
					MSM89XX_PMIC_ANALOG_MICB_1_EN, 0x80,
					0x00);
...
static void msm_anlg_cdc_set_micb_v(struct snd_soc_codec *codec)
{
	...
	if (pdata->micbias.cfilt2_mv > 0) {
		reg_val = VOLTAGE_CONVERTER(pdata->micbias.cfilt2_mv,
				MICBIAS_MIN_VAL, MICBIAS_STEP_SIZE);
		dev_dbg(codec->dev, "cfilt2_mv %d reg_val %x\n",
				(u32)pdata->micbias.cfilt2_mv, reg_val);
	} else {
		reg_val = VOLTAGE_CONVERTER(pdata->micbias.cfilt1_mv,
				MICBIAS_MIN_VAL, MICBIAS_STEP_SIZE);
...
static int msm_anlg_cdc_probe(struct platform_device *pdev)
{
	...
	dev_dbg(&pdev->dev, "%s: micb1_always_on = %d.\n", __func__,
